...
def train(
    ...
    if (fold_idx == 0):
        logger.info("STARTING TRAINING... ") ##Added to be able to make sure log files are correct & don't contain extraneous folds
    ...
    if config['weighted_cross_ent']:
        config["class_weights"] = np.asarray([1.25, 1.1, 1.5, 1.5], dtype=np.float32) ##Changed
    else:
        config["class_weights"] = np.asarray([1.25, 1.1, 1.5, 1.5], dtype=np.float32) ##Changed
